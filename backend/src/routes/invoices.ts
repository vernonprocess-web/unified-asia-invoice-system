...
invoicesApp.post('/', async (c) => {
    ...
    const { quotation_id, customer_id, issue_date, due_date, payment_terms, subtotal, total, notes, items, project_name, is_gst_applicable, gst_amount, po_reference } = body
    ...
    try {
        ...
        // Insert invoice
        const { results } = await c.env.DB.prepare(
            `INSERT INTO invoices (invoice_number, quotation_id, customer_id, issue_date, due_date, payment_terms, subtotal, total, notes, project_name, is_gst_applicable, gst_amount, po_reference) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id`
        ).bind(invoice_number, quotation_id || null, customer_id, issue_date, due_date, payment_terms, subtotal, total, notes, project_name || null, is_gst_applicable ? 1 : 0, gst_amount || 0, po_reference || null).all()
...
invoicesApp.put('/:id', async (c) => {
    ...
    try {
        await c.env.DB.prepare(
            `UPDATE invoices 
       SET quotation_id = ?, customer_id = ?, issue_date = ?, due_date = ?, payment_terms = ?, subtotal = ?, total = ?, notes = ?, project_name = ?, is_gst_applicable = ?, gst_amount = ?, po_reference = ?, updated_at = CURRENT_TIMESTAMP 
       WHERE id = ?`
        ).bind(quotation_id || null, customer_id, issue_date, due_date, payment_terms, subtotal, total, notes, project_name || null, is_gst_applicable ? 1 : 0, gst_amount || 0, po_reference || null, id).run()
